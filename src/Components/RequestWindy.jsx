...
class RequestWindy extends React.Component {
    ...
    requestAirport() {
        ...
        axios
          .all(
            categories.map(cat =>
              axios.get(`https://api.windy.com/api/webcams/v2/list/nearby=${input.lat},${input.lng},100/category=${cat}/orderby=popularity?show=webcams:location,player,category&key=FrImgS6UiLNi4iuMwnlEfZsMOvFGEcad`)
            )
          )
          .then(
            axios.spread(
